...
- **Hive API wiring**: Connect `aws ec2 create-snapshot` and `create-volume --snapshot-id` to viperblock via NATS
- **AMI-backed boot**: `RunInstances` creates volume from AMI's backing snapshot for instant VM launch
