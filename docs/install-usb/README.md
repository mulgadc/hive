...
For this tutorial, a USB drive with at least 8GB of memory is required. Note that flashing the Spinifex ISO onto the USB completely wipes the USB, so ensure no important data is stored on the USB used.
