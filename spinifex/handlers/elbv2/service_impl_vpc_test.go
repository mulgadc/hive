...
// setupTestServicePoolMode creates an ELBv2 service with VPC and DevNetworking=false
// (pool mode) for testing multi-AZ enforcement.
func setupTestServicePoolMode(t *testing.T) (*ELBv2ServiceImpl, *handlers_ec2_vpc.VPCServiceImpl, string) {
	t.Helper()
	opts := &server.Options{
		Host:      "127.0.0.1",
		Port:      -1,
	...
	cfg := &config.Config{Daemon: config.DaemonConfig{DevNetworking: false}}
	elbv2Svc, err := NewELBv2ServiceImplWithNATS(cfg, nc)
	require.NoError(t, err)
	elbv2Svc.SetVPCService(vpcSvc)
	...
	return elbv2Svc, vpcSvc, *vpcOut.Vpc.VpcId
}

func TestCreateLoadBalancer_MultiAZ_RequiredForInternetFacingPoolMode(t *testing.T) {
	svc, vpcSvc, vpcID := setupTestServicePoolMode(t)

	// Single subnet — should fail for internet-facing in pool mode
	sub1 := getTestSubnetID(t, vpcSvc, vpcID, "10.0.1.0/24", "us-east-1a")

	_, err := svc.CreateLoadBalancer(&elbv2.CreateLoadBalancerInput{
		Name:    aws.String("single-az-alb"),
...
func TestCreateLoadBalancer_MultiAZ_DuplicateAZRejected(t *testing.T) {
	svc, vpcSvc, vpcID := setupTestServicePoolMode(t)

	// Two subnets in the same AZ — should fail
	sub1 := getTestSubnetID(t, vpcSvc, vpcID, "10.0.1.0/24", "us-east-1a")
	sub2 := getTestSubnetID(t, vpcSvc, vpcID, "10.0.2.0/24", "us-east-1a")

	_, err := svc.CreateLoadBalancer(&elbv2.CreateLoadBalancerInput{
		Name:    aws.String("dup-az-alb"),
		Scheme:  aws.String("internet-facing"),
		Subnets: []*string{aws.String(sub1), aws.String(sub2)},
	}, testAccountID)
	assert.Error(t, err)
...
func TestCreateLoadBalancer_MultiAZ_TwoAZsAccepted(t *testing.T) {
	svc, vpcSvc, vpcID := setupTestServicePoolMode(t)

	// Two subnets in different AZs — should succeed
	sub1 := getTestSubnetID(t, vpcSvc, vpcID, "10.0.1.0/24", "us-east-1a")
	sub2 := getTestSubnetID(t, vpcSvc, vpcID, "10.0.2.0/24", "us-east-1b")

	out, err := svc.CreateLoadBalancer(&elbv2.CreateLoadBalancerInput{
		Name:    aws.String("multi-az-alb"),
		Scheme:  aws.String("internet-facing"),
		Subnets: []*string{aws.String(sub1), aws.String(sub2)},
	}, testAccountID)
	require.NoError(t, err)
	assert.Len(t, out.LoadBalancers[0].AvailabilityZones, 2)
}

func TestCreateLoadBalancer_MultiAZ_InternalSchemeSkipsValidation(t *testing.T) {
	svc, vpcSvc, vpcID := setupTestServicePoolMode(t)

	// Single subnet with internal scheme — should succeed even in pool mode
	sub1 := getTestSubnetID(t, vpcSvc, vpcID, "10.0.1.0/24", "us-east-1a")

	out, err := svc.CreateLoadBalancer(&elbv2.CreateLoadBalancerInput{
		Name:    aws.String("internal-single-az"),
...
func TestCreateLoadBalancer_MultiAZ_DevModeSkipsValidation(t *testing.T) {
	svc, _ := setupTestServiceWithVPC(t) // dev networking = true

	// Get the pre-created subnet
	subnets, _ := svc.vpcService.DescribeSubnets(&ec2.DescribeSubnetsInput{}, testAccountID)
	subnetID := *subnets.Subnets[0].SubnetId

	// Single subnet with internet-facing in dev mode — should succeed
	out, err := svc.CreateLoadBalancer(&elbv2.CreateLoadBalancerInput{
		Name:    aws.String("dev-single-az"),
		Subnets: []*string{aws.String(subnetID)},
	}, testAccountID)
	require.NoError(t, err)
