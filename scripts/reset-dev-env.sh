...
# --- SSH key (generated as invoking user, not root) ---
SSH_KEY="$INVOKING_HOME/.ssh/spinifex-key"
if [ ! -f "$SSH_KEY.pub" ]; then
    echo "==> Generating SSH key pair ($SSH_KEY)"
    sudo -u "$INVOKING_USER" mkdir -p "$INVOKING_HOME/.ssh"
    sudo -u "$INVOKING_USER" ssh-keygen -t ed25519 -f "$SSH_KEY" -N ""
fi
...
# --- Import AMI ---
echo "==> Importing AMI"
LOCAL_IMAGE="$INVOKING_HOME/images/ubuntu-24.04.img"
ARCH=$(uname -m)
case "$ARCH" in
    x86_64)
        IMG_ARCH="x86_64"
