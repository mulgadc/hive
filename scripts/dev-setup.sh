...
echo ""
echo "Recommended: Use scripts/dev-install.sh for a production-parity setup"
echo "  (systemd, privilege separation, production paths)."
echo ""
echo "  scripts/dev-install.sh  # One-time: build, install, init, start via systemd"
echo "  make deploy             # Daily: rebuild + copy binary + restart all"
echo "  make reinstall          # Changed a .service file? Re-run full setup"
echo ""
echo "Legacy workflow (deprecated, retained for pseudo-multinode CI only):"
echo "  ./scripts/setup-ovn.sh --management"
echo "  sudo ./bin/spx admin init --bind 127.0.0.1 --region ap-southeast-2 --az ap-southeast-2a --node node1 --nodes 1"
