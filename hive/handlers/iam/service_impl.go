...
// MigrateToAccountScoped migrates legacy IAM data (pre-account-scoping) to the
// new account-scoped key format. Runs once on startup. Idempotent — uses
// conditional create so already-migrated entries are skipped safely.
//
// Legacy keys:   "root", "alice", "MyPolicy"
// New keys:      "000000000000:root", "000000000000:alice", "000000000000:MyPolicy"
func (s *IAMServiceImpl) MigrateToAccountScoped() error {
	migrated := 0

	// --- Users: re-key entries without ":" to "000000000000:{key}" ---
	userKeys, err := s.usersBucket.Keys()
	if err != nil && !errors.Is(err, nats.ErrNoKeysFound) {
		return fmt.Errorf("migration: list user keys: %w", err)
	}
	for _, key := range userKeys {
		...
		entry, err := s.usersBucket.Get(key)
		if err != nil {
			slog.Warn("Migration: failed to read user", "key", key, "err", err)
			continue
		}

		var user User
		if err := json.Unmarshal(entry.Value(), &user); err != nil {
			slog.Warn("Migration: failed to unmarshal user", "key", key, "err", err)
			continue
		}

		user.AccountID = GlobalAccountID
		data, err := json.Marshal(user)
		if err != nil {
			slog.Warn("Migration: failed to marshal user", "key", key, "err", err)
			continue
		}

		newKey := GlobalAccountID + ":" + key
		if _, err := s.usersBucket.Create(newKey, data); err != nil {
			if errors.Is(err, nats.ErrKeyExists) {
				slog.Debug("Migration: user already migrated", "key", key)
			} else {
				slog.Warn("Migration: failed to create user", "newKey", newKey, "err", err)
				continue
			}
		}

		if err := s.usersBucket.Delete(key); err != nil {
			slog.Warn("Migration: failed to delete legacy user key", "key", key, "err", err)
		}
		migrated++
	}

	// --- Access keys: add AccountID to entries missing it ---
	akKeys, err := s.accessKeysBucket.Keys()
	if err != nil && !errors.Is(err, nats.ErrNoKeysFound) {
		return fmt.Errorf("migration: list access key keys: %w", err)
	}
	for _, key := range akKeys {
		entry, err := s.accessKeysBucket.Get(key)
		if err != nil {
			slog.Warn("Migration: failed to read access key", "key", key, "err", err)
			continue
		}

		var ak AccessKey
		if err := json.Unmarshal(entry.Value(), &ak); err != nil {
			slog.Warn("Migration: failed to unmarshal access key", "key", key, "err", err)
			continue
		}

		if ak.AccountID != "" {
			continue // already has AccountID
		}

		ak.AccountID = GlobalAccountID
		data, err := json.Marshal(ak)
		if err != nil {
			slog.Warn("Migration: failed to marshal access key", "key", key, "err", err)
			continue
		}

		if _, err := s.accessKeysBucket.Put(key, data); err != nil {
			slog.Warn("Migration: failed to update access key", "key", key, "err", err)
			continue
		}
		migrated++
	}

	// --- Policies: re-key entries without ":" to "000000000000:{key}" ---
	policyKeys, err := s.policiesBucket.Keys()
	if err != nil && !errors.Is(err, nats.ErrNoKeysFound) {
		return fmt.Errorf("migration: list policy keys: %w", err)
	}
	for _, key := range policyKeys {
		...
		if err != nil {
			slog.Warn("Migration: failed to read policy", "key", key, "err", err)
			continue
		}

		newKey := GlobalAccountID + ":" + key
		if _, err := s.policiesBucket.Create(newKey, entry.Value()); err != nil {
			if errors.Is(err, nats.ErrKeyExists) {
				slog.Debug("Migration: policy already migrated", "key", key)
			} else {
				slog.Warn("Migration: failed to create policy", "newKey", newKey, "err", err)
				continue
			}
		}

		if err := s.policiesBucket.Delete(key); err != nil {
			slog.Warn("Migration: failed to delete legacy policy key", "key", key, "err", err)
		}
		migrated++
	}

	if migrated > 0 {
		slog.Info("IAM migration complete", "migratedEntries", migrated)
	}
	return nil
}
